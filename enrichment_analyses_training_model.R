...
str_cor_t3 <- readRDS("data/models/seqwrap_generated_models/trained_untrained_midexercise/filtered_simpler_training_correlation_model.RDS")
...
genes <- readRDS("data/Ct_genes_FPKM.RDS")



ego_df <- enrichGO(gene = unique(less_str_cor_t4$target),
                   universe = mRNA_genes_fpkm$gene_name,
                   keyType = "SYMBOL",
                   OrgDb = org.Hs.eg.db,
                   ont = "BP",
                   pAdjustMethod = "BH",
...
entrez_ids <- bitr(less_str_cor_t4$target, "SYMBOL", "ENTREZID", org.Hs.eg.db)
